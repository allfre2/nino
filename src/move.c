/*--------------------------------------------------------------*
 | Engine name - move.c - Do , Undo typeof, move code.          |
 | License                                                      |
...
/*TODO: debug castling throughoutly
	debug enpassant
	debug promotion !
	add update Pieces and empty
	debug the state of memory in the destination move struct and determine the exact state every time
	Add Zobrist update*/
	 ...
	 Position->castle &= (Castling_rights[K]|Castling_rights[Q]) << Position->turn;
	  ...
	  switch(NPIECE(Position->board[move->from])){
	   case PAWN:
	    ...
	    delta = ABS(delta);
	    if(delta >= 16){ /* move is two step by pawn */
	     ...
	     if(move->to == move->ep && move->ep > 0){ /* move is an en passant capture */

	      ep_xsquare = EP_XSQ(move->ep,Position->turn);//printf("\nen passant xsquare is : %d\n",ep_xsquare);
	     ...
	     else if(move->promote > 0){ /* move is promotion */

	       /*Update Zobrist Hash*/
	       Position->key ^= RandomNumbers[Position->board[move->from]][move->from] ^ RandomNumbers[move->promote][move->from];

	       Position->board[move->from] = move->promote; /*TODO => make sure promote doesn't contain garbage*/
	  ...
	  /*Update Zobrist Hash*/
	  Position->key ^=  RandomNumbers[Position->board[move->from]][move->from]
			  ^ RandomNumbers[Position->board[move->to]][move->to]
			  ^ RandomNumbers[Position->board[move->from]][move->to];
...
/*
		...
		Map(Position);
		Position->key = HashKey(Position);
		if(TMP.key != Position->key){
		printf("\nmaked move: ");printmove(move,Position,_SHORT);
		printf("\nkey before hashkey(): %x\n",TMP.key);

		printf("\nkey after hashkey(): %x\n",Position->key);printBoardinfo(Position);getchar();
		}*/
/*
		memcpy(&TMP,Position,sizeof(Boardmap));
		Map(Position);
		for(int i = 0; i < 64; ++i){
		if(TMP.moves[i] != Position->moves[i]){
		printf("\nmaked move: ");printmove(move,Position,_SHORT);
		printf("\nmoves sq %d after _fastmap: ",i);printBitmap(TMP.moves[i],i);

		printf("\nmoves sq %d after map: ",i);printBitmap(Position->moves[i],i);printBoardinfo(Position);getchar();
		}
		}*/
}
...
/*TODO: EVERITHING!
	Determine the EXACT state of the move struct after the use of this function
	Add Zobrist update*/
...
char delete = Position->board[move->to];
	...
	Position->key ^=  epRandomNumbers[Position->ep] ^ epRandomNumbers[move->ep]
			^ castleRandomNumbers[Position->castle] ^ castleRandomNumbers[move->savedcastle];
	...
	if(move->castle & SHORTCASTLE){
	 ...
	 /*This if moving piece is pawn*/
	  ...
	  /* un-promote to pawn again */
	    ...
	    /*Update Zobrist Hash*/
	    Position->key ^= RandomNumbers[Position->board[move->to]][move->to]
			   ^ RandomNumbers[Position->board[move->to]][move->from];
	    if(move->saved != EMPTY) Position->key ^= RandomNumbers[move->saved][move->to];
	  /*Update bitmaps*/
	  Position->pieces[NPIECE(Position->board[move->to])][Position->turn] &= ~toBit;
	  if(move->saved != EMPTY) Position->pieces[NPIECE(move->saved)][Position->turn^1] |= toBit;
	  Position->pieces[NPIECE(Position->board[move->to])][Position->turn] |= fromBit;
	  /*Update affectedSqs*/
	   affectedSqs |= fromBit | toBit;

	  if(move->saved != EMPTY) Position->empty[Position->turn^1] |= toBit;
	  Position->empty[Position->turn] &= ~toBit;
	  Position->empty[Position->turn] |= fromBit;
...
/*
		Position->key = HashKey(Position);
		if(TMP.key != Position->key){
		printf("\nun-maked move: ");printmove(move,Position,_SHORT);
		printf("\nkey before hashkey(): %x\n",TMP.key);
...
/*
		...
		printf("\nmoves sq %d after map: ",i);printBitmap(Position->moves[i],i);printBoardinfo(Position);printf("\nsaved %d and move->to %d",move->saved,delete);getchar();
		}
		}*/
}
...
/* FIX: This is some really bad sorting done to test the effect of some lousy sorting of the moves */
 int scores[MAXMOVES] = {0};
 int index = 0;
 Move tmp,PvMove;
  memcpy(&PvMove,/*&BEST*/&pv[SortPly].moves[0],sizeof(Move));
 int kingsq,npiece;
 ...
 npiece = NPIECE(Position->board[moves[i].to]);
   //if(MoveCmp(&moves[i],&moves[i]) == 0) printf("\nworks?\n"),getchar();
  if(/*SortPly == current_depth &&*/ thinking && MoveCmp(&moves[i],&PvMove) == 0){
   //printf("\nEqual moves!\n");printmove(&PvMove,Position,_SHORT);printf("\t");printmove(&moves[i],Position,_SHORT);
  // printf("\n\n Current Depth: %d first move:",SortPly); printmove(&PvMove,Position,_SHORT); printf("\n");
 ...
 else if(moves[i].promote > 0){
   memcpy(&tmp,&moves[i],sizeof(Move));
   memcpy(&moves[i],&moves[index],sizeof(Move));
   memcpy(&moves[index],&tmp,sizeof(Move));
   ++index;
 }*/
 else if((npiece == KNIGHT && (KnightSqs[moves[i].to] & Position->pieces[KING][Position->turn^1]))
          || AllRays[moves[i].to] & Position->pieces[KING][Position->turn^1]){ // will be check
   memcpy(&tmp,&moves[i],sizeof(Move));
   memcpy(&moves[i],&moves[index],sizeof(Move));
   memcpy(&moves[index],&tmp,sizeof(Move));
   ++index;
...
/*
 else if(BIT(moves[i].to) & (AllRays[31] | KnightSqs[31])){ // It's a move in the center ??
   memcpy(&tmp,&moves[i],sizeof(Move));
   memcpy(&moves[i],&moves[index],sizeof(Move));
   memcpy(&moves[index],&tmp,sizeof(Move));
   ++index;
 }*//*
 else if(BIT(moves[i].to) & (AllRays[kingsq] | KnightSqs[kingsq])){ // Is an attack to the king
   memcpy(&tmp,&moves[i],sizeof(Move));
   memcpy(&moves[i],&moves[index],sizeof(Move));
   memcpy(&moves[index],&tmp,sizeof(Move));
   ++index;
...
allpieces = Position->empty[turn] | Position->empty[turn^1];
 ...
 switch(npiece){
  ...
  if((Attacks & allpieces) == oking) return 1; else return 0;
}
...
if(nmoves <= 1) return 0;
 ...
 for(int i = 0; i < nmoves; ++i){
   ...
   list[i] = 1000 + see;
  if(Position->board[moves[i].to] != EMPTY){
   if(see > 0) list[i] += 10000000 ;
    else if(see > -200) list[i] += 1000000; else list[i] += 500000;
  }/*else
  if(MoveCmp(&moves[i],&Killers[0]) || MoveCmp(&moves[i],&Killers[1]) ){
    list[i] += 5000000;
   }*/
   ...
   //if(bitto & promoteSqs[turn] && see > 0) list[i] += 200000;
else{
   if(MoveIsCheck(&moves[i],Position)) list[i] += 22000000;
   else{
   if(GetMoveAttacks(Position,&moves[i]) & KingSqs[oksq]) list[i] += see > -200 ? 30000 : 10000;
   if(PieceSqValueT[npiece][moves[i].to] > PieceSqValueT[npiece][moves[i].from]) list[i] += 100;
   if(moves[i].promote != 0) list[i] += see > 0 ? 7000000 : 7000;
    else{
   if(bitto & CenterSqs && see > 0) list[i] += 500;
   }
  } //if(bitto & (AllRays[oksq] | KnightSqs[oksq]) && see > -100) list[i] += 20000;//
 }
   if(moves[i].castle == 0) list[i] += Mhistory[moves[i].from][moves[i].to];
 }
int max,maxi,tmpp;
Move tmp;/*
for(int y = 0; y < nmoves; ++y){
 printf("%d, ",list[y]/1000);
}getchar();*/
 for(int j = 0; j < nmoves; ++j){
   max = -INFINITY;
  for(int k = j; k < nmoves; ++k){
    if(list[k] > max) max = list[k], maxi = k;
  }
if(max > -INFINITY){
   tmpp = list[maxi];
   list[maxi] = list[j];
...
int ValidateMove(Move * move, Move moves[], Boardmap * Position, int nmoves){
	...
	for(i = 0; i < nmoves; ++i){
	 if(moves[i].castle == move->castle ||( moves[i].from == move->from && moves[i].to == move->to )){
	  if(move->promote != 0 && move->promote == moves[i].promote || move->promote == 0 ) break;
	 }
	}
//printf("\njnkkkkkkkkkkkkkkkk %d\n",i);
 if(i < nmoves){
