	...
	for (int p = PAWN; p < 12; ++p)
		for (int i = 0; i < 64; ++i)
			if (p == 12)
				...
				RandomNumbers[p][i] = DefaultRandomNumbers[p][i];
			else
				RandomNumbers[p][i] = rand() % 0xffffffff;

	for (int i = 0; i < 64; ++i)
		if (i == 0)
			epRandomNumbers[i] = 0;
		else if (Default)
			epRandomNumbers[i] = DefaultepRandomNumbers[i];
		else
			epRandomNumbers[i] = rand() % 0xffffffff;

	for (int i = 0; i < 0x10; ++i)
		if (Default)
			castleRandomNumbers[i] = DefaultcastleRandomNumbers[i];
		else
			castleRandomNumbers[i] = rand() % 0xffffffff;
}