...
int verbose;
uint64 higligth;
 _perftResults results[MAX_DEPTH];// = {0,0,0,0,0,0};
char movie[10];
char fen [50];
...
int shit;
	...
	if(INCHECK(args->Position) ){ //return 0;
	 nmoves = GenCheckEscapes(args->Position,moves);
/*if(results[current_depth].nodes > 4865609){
	   printBoardinfo(args->Position);
	 for(int i = 0; i < nmoves; ++i)
	   printf("  "),printM(&moves[i],args->Position);
 GetFENof(args->Position,fen);
 printf("\n%s",fen);getchar();}//*/
	}
	else nmoves = args->genfn(args->Position,moves);
#ifdef ONLYLEGALGEN
	if(ply <= 1){
	uint64 allpieces = args->Position->empty[turn] | args->Position->empty[turn],
	 ...
	 for(int i = 0; i < nmoves; ++i){
	  //if(GetBishopRay(moves[i].to,KingSq) & allpieces);
	  shit = 0;
	 args->makefn(&moves[i],args->Position);
	 if(CHECK(args->Position)) {
	   shit = 1;
	   printf("shitttttttttttttttttttttttt\n\n\n\n");
...
/*
	  if(moves[i].castle) results[current_depth].castles++;
	   else{
	     if(args->Position->board[moves[i].to] != EMPTY) results[current_depth].captures++;
	     if(moves[i].promote > 0) results[current_depth].promotions++;
	     if(NPIECE(args->Position->board[moves[i].to]) == PAWN){
	     if(args->Position->ep == moves[i].to) results[current_depth].ep++,results[current_depth].captures++;
	  }
	...
	if(args->Position->pieces[KING][args->Position->turn] & args->Position->attackd[args->Position->turn^1])
	 {
	  ...
	  if(moves[i].castle) results[ply].castles++;/* printBoardinfo(args->Position);getchar();*/
	   else{
	  if(moves[i].saved != EMPTY) results[ply].captures++;
	  if(moves[i].promote > 0) results[ply].promotions++;
	  if(NPIECE(args->Position->board[moves[i].to]) == PAWN){
	  if(moves[i].ep == moves[i].to) results[ply].ep++,results[ply].captures++;
	  }

}
	  /*results[ply].nodes += */_perft(ply - 1, args, results);
	 }else{ higligth = 0;
 //printf("fuck\n"),printBoardinfo(args->Position),printM(&moves[i],args->Position),getchar();
...
Boardmap Position;
Move moves[MAXMOVES];
double genfinal = 0,capturefinal = 0,mapfinal = 0,makefinal = 0,evalfinal = 0,engine_cicle = 0,
quiescencefinal = 0,seefinal = 0;
...
for(int i = 0; i < testcases; ++i){
 nmoves = GenMoves(&Position,moves/*,PSEUDOLEGAL*/);
}
f = clock();
printf("\n--> Generated %d moves in %f seconds.\n",nmoves*testcases,(f-i)/(CLOCKS_PER_SEC));

genfinal = (nmoves*testcases)/(((f-i)/CLOCKS_PER_SEC));
printf("==> Generates ~ %.2f moves per second",genfinal);

for(int i = 0; i < nmoves; i++){
 printf("\n%d. ",i+1); printmove(&moves[i],&Position,_SHORT);
}

i = clock();
for(int i = 0; i < testcases; ++i){
 nmoves = GenCaptures(&Position,moves/*,PSEUDOLEGAL*/);
}
f = clock();
printf("\n--> Generated %d captures in %f seconds.\n",nmoves*testcases,(f-i)/(CLOCKS_PER_SEC));

capturefinal = (nmoves*testcases)/(((f-i)/CLOCKS_PER_SEC));
...
f = clock();
 printf("\n--> fastMap'd %d positions in %f seconds.\n",testcases,(f-i)/(CLOCKS_PER_SEC));
mapfinal = (testcases)/(((f-i)/CLOCKS_PER_SEC));
...
makefinal = (nmoves*testcases)/((f-i)/CLOCKS_PER_SEC);
...
evalfinal = (testcases)/(((f-i)/CLOCKS_PER_SEC));
...
f = clock();
 printf("\n--> see()'d %d positions in %f seconds.\n",testcases*64,(f-i)/(CLOCKS_PER_SEC));
seefinal = (testcases*64)/(((f-i)/CLOCKS_PER_SEC));
...
quiescencefinal = (testcases)/(((f-i)/CLOCKS_PER_SEC));
printf("==> Quiescences ~ %.2f times per second",quiescencefinal);

engine_cicle = (evalfinal+makefinal+((genfinal/*+genfinal+capturefinal/3*/)))/3;
...
void printmove(Move * move, Boardmap * Position, int n){
	...
	else ; /*Descriptive notation ... LAME*/
}
...
int printPV(Line * pv, Boardmap * Position){
 ...
 for(int i = 0; i < pv->length; ++i){
  ...
  if(MoveIsCheck(&pv->moves[i],&TMP)) printf("+");
  printf(" ");
...
//(void) signal (SIGINT,debugSignals);
int ntokens = 0;
exitXboard = 1;
int no_promt = 0;
...
else if(strcmp(tokens[0],"verbose") == 0){ /* --flag-- each command prints a lot of info*/
 NOTIMPLEMENTED;
}
...
int depth;
 ...
 current_depth = depth;
 _perftArgs args = {&Position,makefn[1],unmakefn[1],GenMoves,GenCaptures};
...
printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
...
else if(strcmp(tokens[0],"reset") == 0){ /* reset board position */
 NOTIMPLEMENTED;
}
 ...
 printBoardinfo(&Position);
  ...
  for(int i = 0; i < nmoves; ++i){
   printf(" %d) ,",i); printM(&moves[i],&Position);
  }
}
...
if(INCHECK(&Position)) nmoves = GenCheckEscapes(&Position,moves);
  else
#endif
nmoves = GenMoves(&Position,moves);
...
else if(strcmp(tokens[0],"eval") == 0){ /* takes arguments saying the kind of eval to be done */
 printf("\n => %d",Eval(&Position));
}
...
int ncaptures;
ncaptures = GenMoves(&Position,m); //GenCaptures(&Position,m);
for(int n = 0; n < ncaptures; ++n){

see_score = SEE(&m[n],&Position);
printBoardinfo(&Position);
printf("\n => SEE score for move: "); printmove(&m[n],&Position,_SHORT);printf(" -> %d\n",see_score);getchar();
}
...
/***delete***/
...
else if(strcmp(tokens[0],"set") == 0 || strcmp(tokens[0],"=") == 0){ /*if has arguments tries to use the argument as a fen position else sets the stating pos*/
 ...
 }else SetfromFEN("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq -",&Position);
}
...
printf("nbitmap: %d, ntokens: %d",nbitmap,ntokens);getchar();
 switch(nbitmap){
  case 1:
	printf("\nwhite attacked:");
  ...
  case 2:
	printf("WHITE");
	printBoardinfo(&Position);
	printBitmap(Position.empty[WHITE],-1);getchar();
	printf("\nBLACK");
	printBoardinfo(&Position);
 ...
 if(TTable != NULL){ freeTTable(); TTable = NULL;}
 int sze = TTABLESIZE;
 if(ntokens > 1){
...
else if(strcmp(tokens[0],"search") == 0 || strcmp(tokens[0],"play") == 0
|| strcmp(tokens[0],";") == 0 || strcmp(tokens[0],"'") == 0){
/*DELETE ME*/
	double EBF; /*effective branching factor*/
/***************/

flags = 0; /* Thread shitttt*/
memset(Scores,0,sizeof(int)*MAX_DEPTH);
 ...
 if(BEAST.flags != NOMOVE){ makefn[1](&BEAST,&Position);
  memcpy(&moveHistory[nhistory],&BEAST,sizeof(Move));
  nhistory += (nhistory >= 99 ? 0: 1);
 }
 }
 printBoardinfo(&Position);
 //printPV(ply,Position.turn);
 printf("\n");
 printPV(&pv[current_depth],&Position);
 if(BEAST.flags != NOMOVE){ printf("\n => move: ");
 ...
 turn = Position.turn;
 if(turn == WHITE){
  if(INFINITY - sc < MAX_DEPTH){ matestr = '#'; sc = INFINITY - sc; sc++;  sc /= 2;}
 }else{
  if(-INFINITY + sc > -MAX_DEPTH){ matestr = '#'; sc = -INFINITY + sc; sc *= -1; sc++;  sc /= 2;}
 }
 printf("\n => Score: %c%d\n",matestr,sc);
...
//memset(Scores,0,sizeof(int)*MAX_DEPTH);
memset(Mhistory,0,sizeof(short)*64*64);
if(ntokens > 1) sscanf(tokens[1],"%f",&TimeToThink);//atoi(tokens[1]);
...
INIT_MUTEX(StopIterate);
 ...
 ft = clock();
  ...
  if(BEAST.flags != NOMOVE){  makefn[1](&BEST,&Position);
	/*if(INCHECK(&Position)) higligth = Position.attackd[Position.turn^1];
         else//*/ higligth = 0;
   if(nhistory < 99){
 ...
 printBoardinfo(&Position);
 //print_PV(&_PV,&Position);
 //printPV(&pv[current_depth],&Position);
 ///*calculate EBF*/
  if(current_depth > 1) EBF = GNodes[1];
  else EBF = (GNodes[current_depth -1] + GNodes[current_depth]) / 2;
   //printf("\nhere\n");
 if(BEAST.flags != NOMOVE){ printf("\n move: ");printmove(&BEST,&game.Position,666);}
 else printf("\n %s",INCHECK(&game.Position) ? turn == BLACK? "White Mates 1-0." :  "Black Mates 0-1." : "Stalemate 1/2-1/2.");
 char matestr;
 matestr = '\0';
 int sc = Scores[current_depth-1];
  if(sc > (INFINITY-MAX_DEPTH) || sc < (-(INFINITY-MAX_DEPTH)) ){ matestr = '#'; sc = INFINITY - abs(sc); sc++;  sc /= 2;}
 printf("\n => Score: %c%d\n",matestr,sc);
 ...
 int testcases;
 testcases = 1000000;
 if(ntokens > 1) testcases = atoi(tokens[1]);
...
system(syscommands);
 ...
 Xboard();
	...
	uint64 pieces,square,allpieces,allmoves,movesq,KingSqRays,bitspace,sliders;
	pieces = Position.empty[Position.turn];
	LOG2(Position.pieces[KING][turn],ksq);
	//sliders = Position->empty[]
	...
	printf("\n %s",algebraic[sq]); printBoardinfo(&Position);printBitmap(KingSqRays,-1);printBitmap( KingSqRays ^ 63,-1);getchar(); LSBCLEAR(pieces);
}// while
}
 ...
 if(ntokens > 1){
 sq = CHARSQINT(tokens[1][0], tokens[1][1]);
 if(sq > 63 || sq < 0) printf("\n\t(!) Invalid square. \n");
 else{
  if(ntokens > 2) Position.board[sq] = GetpieceN(tokens[2][0]);
 else
   Position.board[sq] = EMPTY;
 //memcpy(btmp,Position.board,64*sizeof(char));
 //Initboard(&Position,btmp);
 mapfn[0](&Position);
...
int epdTest(char * filename){
 ...
 FILE * log = fopen("log","w"); if(log == NULL){ printf("\n => Couldn;t open log file\n"); return -1;}
 if(epdfile == NULL){
  printf("\nCan't Open file: %s \n",filename);
  return -1;
 }
  ...
  while(fgets(buffer,254,epdfile) != NULL){
   ...
   for(int i = 0; buffer[i] != 0 && buffer[i] != '\n'; ++i){
    if(buffer[i] == ';'){
     ...
     int ansi = 0;
     while(ansi < 9 && back < i) Answer[ansi++] = buffer[back++];
     Answer[ansi] = '\0'; /* Null Terminator */
    // printf("%s",Answer); getchar();
     break;
    }
   }
     /* Prepare for thinkinng */
     memset(Scores,0,sizeof(int)*MAX_DEPTH);
     ...
     printf("\n move: ");printmove(&BEST,&game.Position,SAN/*666*/);
     printf("\n Correct Move: %s\n",Answer);
