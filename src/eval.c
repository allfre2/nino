...
{
 0, 0,  0,  0,  0,  0,  0,  0, /* Never going to be a pawn there ?*/
...
/* Pawn Evaluation */
 ...
 /* King attack and king safety */
    ...
    uint64 kings;
    int count;/*
     LOG2(Position->pieces[KING][WHITE],sq);
	kings = Position->attackd[BLACK] & KingSqs[sq];
	PIECECOUNT(kings,count);
     score -= count * 10;
     LOG2(Position->pieces[KING][BLACK],sq);
	kings = Position->attackd[WHITE] & KingSqs[sq];
...
int SEE(Move * move, Boardmap * Position){
 ...
 if(Position->board[move->to] == EMPTY) score  = 0;
 else score = Relative_Piece_Value[piece];
// printf("\n Score after getting first piece: %c => %d\n",Pieces[piece],score); getchar();
 tosqBit = BIT(move->to);
...
/* Loop to see wich pieces are actually attacking the move->to square */
 ...
 piecesBits[turn^1] = Attackers & Position->empty[turn^1];
  ...
  //printBitmap(Attackers,-1); printf("\n");
 //printBitmap(Attackers,0);
 nextPiece = BIT(move->from);
 /* Start swapping the pieces*/
 turn ^= 1;
 while(piecesBits[turn] && piecesBits[turn^1]){
 /* Check Xrays */
  if(LONG_RANGE(piece) || piece == PAWN){ /* If previous piece is bishop, rook, queen or pawn check behind it */
   bitSpace = BITSPACE(tosqBit,nextPiece); /* Calcular el bitmap de los bits entre la casilla de captura y la pieza*/
   LOG2(nextPiece,sq);
     ...
     (Position->pieces[BISHOP][turn^1] | Position->pieces[ROOK][turn^1] | Position->pieces[QUEEN][turn^1]))
   {
    ...
    //printBitmap(discoveredPiece,sq);
   }
  }
 ...
 * capture since it was an invalid move to consider. (king captures a defended piecie)*/
  }
   ...
   if(piece > KNIGHT && piece < KING){ // ????? When pawns get out of way also goes ?
   /* SLIDING PIECE: check behind to se if there is another piece forming a battery */

   }
   turn ^= 1;
  //printf("\n %s removed a %c",turn == Position->turn? "\t>":"<",Pieces[piece]);

  // printBitmap(pieces[Position->turn],move->to);
  // printBitmap(pieces[Position->turn^1],move->to);
