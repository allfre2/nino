/*--------------------------------------------------------------*
 | Engine name - notation.c - fen, pgn and other notation code. |
...
int SetfromFEN(char * fen, Boardmap * Bmap){
	...
	for(int i = 0; fen[i] && i < MAXFENLEN && field != END; ++i){
	 if(fen[i] == FIELDELIMITER){ 
	  //while(fen[i] == FIELDELIMITER && i < MAXFENLEN) ++i;
		field++; 
		continue;
	 }
	 switch(field){
	  case BOARD:
	   ...
	   if(ISDIGIT(fen[i])){
	    digit = '0';
	    while(fen[i] > digit && digit < '9'){ ++index; ++digit;}
	   }
	   else if(fen[i] != ROWDELIMITER) Bmap->board[index++] = GetpieceN(fen[i]);
	  break;
	  case TURN:
	   if(LOWERCASE(fen[i]) == 'w') Bmap->turn = WHITE;
	   else Bmap->turn = BLACK;
	  break;
	  case CASTLING:
	   if(fen[i] == EMPTYFIELD) Bmap->castle = Castling_rights[NONE];
	    else
		Bmap->castle |= Castling_rights[ Select(fen[i], "-KkQq") ];
	  break;
	  case EP:
	   if(fen[i] == EMPTYFIELD) break;
	   if(fen[i+1] && fen[i+1] != FIELDELIMITER)
	    Bmap->ep = CHARSQINT(fen[i],fen[i+1]);
	  break;
	  case HALFMOVECLOCK: // TODO
	  break;
	  case FULLMOVENUM: // TODO
	  break;
	 }
	}

	/*if Validate position == OK return GOOD else return ERROR CODE*/
 Map(Bmap);
...
/*--> BOARD FIELD */
	...
	for(int i = 0; i <= 64 && index < MAXFENLEN-3; ++i){
	 if(i > 0 && (i % 8) == 0){
	  if(digit > '0'){fen[index++] = digit; digit = '0';} 
	  if(i != 64 ) fen[index++] = ROWDELIMITER;
	  else break;
	 }
...
/*--> CASTLING */
	...
	if(Bmap->castle == Castling_rights[NONE])
	 ...
	 if(Bmap->castle & Castling_rights[K]) fen[index++] = castlingchars[K];
	 if(Bmap->castle & Castling_rights[Q]) fen[index++] = castlingchars[Q];
	 if(Bmap->castle & Castling_rights[k]) fen[index++] = castlingchars[k];
	 if(Bmap->castle & Castling_rights[q]) fen[index++] = castlingchars[q];
	}
...
int LoadPgnFromFile(void * fileptr, Pgn * Pgns, int limit){
	...
	while(games < limit && fgets(line,MAXPGNLINE,file) != NULL){
	 ...
	 for(int i = 0; line[i]; ++i){
	  if(errorflags) return -errorflags;
	  if(line[i] == '['){ 
/*	   errorflags |= ;*/
	   tagindex = 0;
	   while(line[i] && Select(line[i],"- \t") != 0) ++i;
	   while(line[i] && Select(line[i],"- \t") == 0) tag[tagindex++] = line[i++];
	   while(line[i] && Select(line[i],"-\"") == 0) ++i;
	   tagn = wichTag(tag);
	   if(tagn != -1){ Pgns[games];}
	  } else
	  if(line[i] == '('){
	  } else
	  if(line[i] == '{'){ /* braquet comment */
	  } else
	  if( ( line[i] == '%' && i == 0 ) || line[i] == ';') continue; /* end of line comment*/
	    else
	  if(line[i]){}
	 }
	}
...
int SANmove(Move * move, Boardmap * Position, char * SAN){
	...
	if(1); /*check if there is ambiguity for this piece or if move is enpassant */
	...
	/* then if it is a pawn just put the column char there*/
	 if(Position -> board [move->to] != EMPTY) SAN[index++] = 'x'; /*check if move is capture. (put an `x` there)*/
 ...
 if(Position -> pieces[KING][Position->turn^1] & Position -> attackd[Position->turn]) SAN[index++] = '+';
 SAN[index] = '\0';
 return index;
}
 ...
 */
	...
	for(i = 0; input[i] && i < 255; ++i){
	 ...
	 // printf("\nfound valid char at %d\n",i);
	  break;
	 }
	}
	if(i > 240) return -1; /* Whatever ... */

	move->from = CHARSQINT((input[i] | 0x20),(input[i+1] | 0x20));
	move->to = CHARSQINT((input[i+2] | 0x20),(input[i+3] | 0x20));

	//printf("\nComputed: from -> %d, to -> %d\n",move->from,move->to);

	if(move->from < 0 || move->from > 63 || move->to < 0 || move->to > 63) return -1;
